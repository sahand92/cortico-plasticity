...
%       %quiver(X(index),Y(index),transpose(dUdt),transpose(dVdt),0.3,'blue')
     ...
     lw=find(X>0.1 & X<0.5 & Y>0.4 & Y<0.75);
     ...
     hold on
     ls=find(X>0.8 & X<1 & Y>-0.15 & Y<-0.0);
     quiver(X(ls),Y(ls),transpose(dUdt(ls)),transpose(dVdt(ls)),0.5,'blue')
     hold on
     quiver(mean(X(ls)),mean(Y(ls)),mean(dUdt(ls)),mean(dVdt(ls)),0.5,'LineWidth',2,'MaxHeadSize',2,'color','blue')
