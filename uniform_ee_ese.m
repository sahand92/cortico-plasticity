...
l=find(Gee>0+n*i&Gee<0+n*(i+1) & Gese>0+n*j&Gese<0+n*(j+1),1);
if isempty(l)==1;
     l=0;  
 end 
 %disp(l)
...
radius=sqrt(dGeedt.^2 + dGesedt.^2);
dGe=dGeedt./radius;
dGese=dGesedt./radius;

dGe_s=smoothn(dGe);
dGese_s=smoothn(dGese);
rad=sqrt(dGe_s.^2 + dGese_s.^2);
dGe_sm=dGe_s./rad;
dGese_sm=dGese_s./rad;

quiver(Gee(index),Gese(index),dGe_sm,dGese_sm,0.3,'blue')