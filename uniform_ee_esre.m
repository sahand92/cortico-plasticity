...
% M=zeros(size(M))
% M=[newX newY];
n=1;  %square size is n x n
%index=zeros(400,2);

for i=0:ceil(20/n)
    for j=0:ceil(60/n)
   
l=find(Gee>0+n*i&Gee<0+n*(i+1) & Gerse>0-n*j&Gerse<0-n*(j-1),1);
if isempty(l)==1;
     l=0;  
 end 
 %disp(l)
...
dGersedt(c,1)=real(dG_erse);

end

radius=sqrt(dGeedt.^2 + dGersedt.^2);
dGe=dGeedt./radius;
dGerse=dGersedt./radius;

dGe_s=smoothn(dGe);
dGerse_s=smoothn(dGerse);
rad=sqrt(dGe_s.^2 + dGerse_s.^2);
dGe_sm=dGe_s./rad;
dGerse_sm=dGerse_s./rad;

quiver(Gee(index),Gerse(index),dGe_sm,dGerse_sm,0.3,'blue')