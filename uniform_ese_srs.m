...
% M=zeros(size(M))
% M=[newX newY];
n=0.8;  %square size is n x n
%index=zeros(400,2);

for i=0:ceil(85/n)
    for j=0:ceil(7/n)
   
l=find(Gese>0+n*i&Gese<0+n*(i+1) & Gsrs>0-n*j&Gsrs<0-n*(j-1),1);
if isempty(l)==1;
     l=0;  
 end 
 %disp(l)
...
dGesedt(c,1)=real(dG_ese);

end

radius=sqrt(dGesedt.^2 + dGsrsdt.^2);
dGese=dGesedt./radius;
dGsrs=dGsrsdt./radius;

dGese_s=smoothn(dGese);
dGsrs_s=smoothn(dGsrs);
rad=sqrt(dGese_s.^2 + dGsrs_s.^2);
dGese_sm=dGese_s./rad;
dGsrs_sm=dGsrs_s./rad;

quiver(Gese(index),Gsrs(index),dGese_sm,dGsrs_sm,0.1,'blue')